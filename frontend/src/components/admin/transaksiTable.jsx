    ...
    const getTransaksi = async () => {
        try {
            ...
            if(error.response) {
                ...
                setError(error.response.data.msg || 'Terjadi kesalahan saat mengambil data');
            ...
            <table className="table is-fullwidth is-striped">
                ...
                <tbody>
                    {filteredTransaksi.length === 0 ? (
                        <tr>
                            <td colSpan="8" className="has-text-centered">Tidak ada data transaksi yang perlu dikonfirmasi</td>
                        </tr>
                    ) : (
                        filteredTransaksi.map((t) => (
                            <tr key={t.id_transaksi}>
                                ...
                                <td>{t.penerbangan?.kode_penerbangan} - {t.penerbangan?.maskapai}</td>
                                <td>{t.jumlah_tiket}</td>
                                <td>Rp {t.total_harga.toLocaleString()}</td>
                                <td>
                                    <span className={`tag ${getStatusColor(t.status_pembayaran)}`}>
                                        {t.status_pembayaran === 'pending' ? 'Menunggu Pembayaran' :
                                         t.status_pembayaran === 'berhasil' ? 'Pembayaran Berhasil' : 'Pembayaran Gagal'}
                                    </span>
                                </td>
                                <td>
                                    <span className={`tag ${t.status_tiket === 'aktif' ? 'is-success' : 'is-danger'}`}>
                                        {t.status_tiket === 'aktif' ? 'Aktif' : 'Tidak Aktif'}
                                    </span>
                                </td>
                                <td>
                                    {t.status_pembayaran !== 'pending' && t.status_tiket === 'tidak aktif' && (
